...
const handleMainOrder = (order, callback) => {
    if (order === ORDER_MAP.ADD) {
        state = STATE_MAP.ADD;
        callback(HINTS_MAP.ADDITION_INFO);
    }
    if (order === ORDER_MAP.PRINT) {
        state = STATE_MAP.PRINT;
        callback(HINTS_MAP.STU_INPUT_INFO);
    }
    if (order === ORDER_MAP.QUIT) {
        rl.close();
    } else {
        callback();
    }
